...
exports.handler = async (event) => {
    ...
    try {
        ...
        const HF_TOKEN = process.env.HF_TOKEN; // Берем из настроек Netlify

        const postData = JSON.stringify({
            inputs: `Реши тест. Напиши ТОЛЬКО цифры правильных ответов через запятую. Вопрос: "${question}" Варианты: ${options.map((opt, i) => i + ": " + opt).join(", ")} Ответ:`,
            parameters: { max_new_tokens: 10, return_full_text: false, wait_for_model: true }
        });
        ...
        console.log("HF_LOG_NEW_CODE:", JSON.stringify(result));
